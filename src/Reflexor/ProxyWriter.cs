...
internal static class ProxyWriter
{
    extension(ref IndentedStringBuilder writer)
    {
        ...
        {
            var identifiers = IndentedStringBuilder.GetProxyIdentifiers(proxy);
            ...
            writer.WriteInstanceMembers(proxy, identifiers);

            foreach (var property in proxy.Properties)
            {
                writer.WriteProperty(proxy, identifiers, property);
                writer.WriteLine();
            }

            foreach (var method in proxy.Methods)
            {
                writer.WriteMethod(proxy, identifiers, method);
                writer.WriteLine();
            }
        ...
        private void WriteInstanceMembers(Proxy proxy, ProxyIdentifiers identifiers)
        {
            if (proxy.IsStatic)
            {
                return;
            ...
            writer.Indent++;
            writer.Write(identifiers.TargetFieldName);
            writer.WriteLine(" = target ?? throw new System.ArgumentNullException(nameof(target));");
            writer.Indent--;
            writer.WriteLine("}");
            writer.WriteLine();

            writer.Write("private readonly void ");
            writer.Write(identifiers.GuardMethodName);
            writer.WriteLine("()");
            writer.WriteLine("{");
            writer.Indent++;
            writer.Write("if (");
        ...
        private void WriteProperty(Proxy proxy, ProxyIdentifiers identifiers, Property property)
        {
            writer.Write("public ");
        ...
        private void WritePropertyGuard(Proxy proxy, ProxyIdentifiers identifiers, Property property)
        {
            ...
            ImmutableArray<Parameter> parameters)
        {
            ...
            var targetParameterName = IndentedStringBuilder.GetGeneratedIdentifier("target", parameters.Select(static parameter => parameter.Name));
            ...
            if (isStaticMember && proxy.IsStatic)
            {
                ...
                writer.Write("\")] object? ");
                writer.Write(targetParameterName);
                wroteParameter = true;
            }
        ...
        private static ProxyIdentifiers GetProxyIdentifiers(Proxy proxy)
        {
            var memberNames = proxy.Properties
                .Select(static property => property.Name)
                .Concat(proxy.Methods.Select(static method => method.Name))
                .ToList();

            var targetFieldName = IndentedStringBuilder.GetGeneratedIdentifier("_target", memberNames);
            var guardMethodName = IndentedStringBuilder.GetGeneratedIdentifier(
                "ThrowInvalidOperationIfNotInitialized",
                memberNames.Append(targetFieldName));

            return new ProxyIdentifiers(targetFieldName, guardMethodName);
        }

        private static string GetGeneratedIdentifier(string preferredName, IEnumerable<string> disallowedNames)
        {
            var disallowed = new HashSet<string>(disallowedNames, StringComparer.Ordinal);
            var name = IndentedStringBuilder.GetSafeIdentifier(preferredName);
            var suffix = 1;

            while (disallowed.Contains(name))
            {
                name = IndentedStringBuilder.GetSafeIdentifier($"{preferredName}{suffix}");
                suffix++;
            }
    ...
    private readonly record struct ProxyIdentifiers(

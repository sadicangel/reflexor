...
public static class ProxyWriter
{
    extension(ref IndentedStringBuilder writer)
    {
        ...
        {
            var identifiers = GetProxyIdentifiers(proxy);
        ...
        {
            var accessorName = GetGeneratedIdentifier(
            ...
            ImmutableArray<Parameter> parameters)
        {
            ...
            var targetParameterName = GetGeneratedIdentifier("target", parameters.Select(static parameter => parameter.Name));
        ...
        private static ProxyIdentifiers GetProxyIdentifiers(Proxy proxy)
        {
            ...
            var targetFieldName = GetGeneratedIdentifier("_target", memberNames);
            var guardMethodName = GetGeneratedIdentifier(
        ...
        private static string GetGeneratedIdentifier(string preferredName, IEnumerable<string> disallowedNames)
        {
            ...
            var name = GetSafeIdentifier(preferredName);
            var suffix = 1;

            while (disallowed.Contains(name))
            {
                name = GetSafeIdentifier($"{preferredName}{suffix}");
                suffix++;
            }
